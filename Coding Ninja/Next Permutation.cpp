...
{
    int ind = -1 ;    // Initialize the variable "ind" with -1
    for(int i = n - 2 ; i >= 0 ; i--)    // Traverse the array from right to left
    {
        if(permutation[i] < permutation[i + 1])    // Check if element is smaller than next element
        {
            ind = i ; break ;    // if yes then assign the value of i to ind and break
        }
    }
    if(ind == -1) 
    {
        reverse(permutation.begin(), permutation.end());    // Reverse the permutation array
        return permutation ;    // Return the permutation array
    }
    int j = n - 1;    // Initialize j to n-1
    while(permutation[j] <= permutation[ind])    // Traverse the array from right to left
        j--;
    swap(permutation[j], permutation[ind]);    // Swap the elements at index j and ind
    reverse(permutation.begin() + ind + 1, permutation.end());    // Reverse the permutation array
    return permutation ;    // Return the permutation array
}
