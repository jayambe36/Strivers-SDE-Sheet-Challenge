...
{
    int n = input.size() ;    // Initialize variable n as size of input string
    int maxCnt = 0 ;    // Initialize maxCnt as 0
    unordered_set<char> hashSet ;    // Unordered set to store unique characters

    for(int start = 0, end = 0 ; start < n ; start++)    // Loop through input string
    {
        char accept = input[start] ;    // Take the current character of input string

        while(end <= start && hashSet.find(accept) != hashSet.end() )    // Check if the current character is already present in unordered set or not
        {
            char discard = input[end] ; end++ ;    // Take the character from left hand side of substring
            hashSet.erase(discard) ;    // Delete that character from unordered set
        }
        
        hashSet.insert(accept) ;    // Insert current character in unordered set
        maxCnt = max(maxCnt , start - end + 1) ;    // Update maximum length of substring
    }

    return maxCnt ;    // Return maximum length of substring
}
