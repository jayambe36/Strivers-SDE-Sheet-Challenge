#include <bits/stdc++.h>    
using ll = long long int ;    // Define the datatype as long long int

ll binaryExponentiation(ll base , ll n , ll MOD)    // function for binary exponentiation
{
	if(n == 0)  return 1 ;    // If n is zero, return 1
	...
	while(n > 0)
	{
		if(n % 2 == 1)
		{
			ans *= base ;  ans %= MOD ;    // if n is odd, multiply base with ans and take mod of it
		}
		base *= base ;  base %= MOD ;    // divide n by 2
		n /= 2 ;
	}
	return (ans % MOD) ;    // return ans mod MOD
}

int modularExponentiation(int x, int n, int m)    // function for modular exponentiation
{
	return (int) binaryExponentiation( (ll) x , (ll) n , (ll) m) ;    // call binary exponentiation function
}
