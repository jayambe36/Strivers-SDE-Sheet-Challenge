...
Node *findMiddle(Node *head)    // Function for finding middle of the list
{
    Node *slow = head, *fast = head;    // Initialize two pointers fast and slow and point both to head

    while(fast && fast->next)    // while loop body
    {
        slow = slow->next;    // Move slow pointer by one step
        fast = fast->next->next;    // Move fast pointer by two steps
    }

    return slow;    // Return the value of slow pointer to the caller function
}
