...
Node * findIntersection(Node *firstHead, Node *secondHead)    // function to find the intersection of two linked lists
{
    if(firstHead == NULL || secondHead == NULL)  return NULL ;    // If first head or second head is NULL, return NULL

    Node *p = firstHead ; Node *q = secondHead ;    // initialize node p and q with first and second head respectively

    while(p != q)    // loop until p and q become equal
    {
        p = (p == NULL) ? secondHead : p->next;    // If p is equal to NULL, assign second head to p, else point to next node
        q = (q == NULL) ? firstHead : q->next;    // If q is equal to NULL, assign first head to q, else point to next node
    }

    return p ;    // return p after loop
}
