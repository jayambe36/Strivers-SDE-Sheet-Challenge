Node *rotate(Node *head, int k)    // Function to rotate the linked list
{
    if(!head || !head -> next || k == 0)    // Check if head or head next is NULL or k is 0
  return head ;

    Node *p = head ;  int n = 1 ;    // Initialize p with head and n with value 1

    while(p -> next != NULL)    // Loop until p->next is NULL
    {
         p = p -> next ; n++ ;    // move to next element
    }     
    ...
    Node *rotHead = p -> next ;   p -> next = NULL ;    // Initialize rotHead with p->next and p->next with NULL
