bool searchMatrix(vector<vector<int>>& mat, int target) {    // function definition of searchMatrix
        int m = mat.size();    // initialize m with number of rows in mat
        int n=mat[0].size();    // initialize n with number of columns in mat
        int startRow  = 0;    // start row position
        int startCol = n-1;    // start column position

        while (startRow < m && startCol >= 0) {    // check if startRow less than m and startCol greater than 0
            int val = mat[startRow][startCol];    // Initialize val with value at startRow & startCol position
            if(val == target) return true;    // Check if val is equal to target
            else if (val < target) startRow++;    // If val is less than target, increment startRow
            else startCol--;    // else decrement startCol
