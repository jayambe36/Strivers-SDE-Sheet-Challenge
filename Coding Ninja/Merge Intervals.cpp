...
{
    int n = intervals.size() ;    // Initialize n with size of intervals
    
    vector<vector<int>> ans ;    // Initialize ans vector

    sort(intervals.begin() , intervals.end() ) ;    // Sort intervals in ascending order

    int start = intervals[0][0] ;  int end = intervals[0][1] ;    // Initialize start and end variables with first element of intervals

    for(int i = 1 ; i < n ; i++)    // Iterate using for loop
    {
        int newSt = intervals[i][0] ; int newEnd = intervals[i][1] ;    // Initialize newSt and newEnd with ith element of intervals

        if(newSt <= end)    // Check if newSt is less than end
        {
            end = max(end , newEnd) ;    // Update end with maximum of end and newEnd
        }

        else    // else body
        {
            ans.push_back({start , end}) ;    // Push the value of start and end in ans

            start = newSt ; end = newEnd ;    // Update start and end with newSt and newEnd
        }
    }
