...
{
	int majorEle = 0 ; int cnt = 0 ;    // Initialize majorEle and cnt as 0

	for(int i = 0 ; i < n ; i++)
	{
		if(cnt == 0)  majorEle = arr[i] ;    // If cnt is 0, assign arr[i] to majorEle

		cnt += (majorEle == arr[i]) ? 1 : -1 ;    // Increment cnt if majorEle is equal to arr[i], otherwise decrement cnt
	}

	int freq = 0 ;    // Initialize frequency as 0

	for(int i = 0 ; i < n ; i++)
	{
		if(majorEle == arr[i])  freq += 1 ;    // Find the frequency of majorEle
	}

	return freq * 2 > n  ? majorEle : -1 ;    // If frequency is greater than n/2, return the majorElement, otherwise -1
}
