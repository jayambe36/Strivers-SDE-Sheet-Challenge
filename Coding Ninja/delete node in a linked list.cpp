void deleteNode(LinkedListNode<int> * node)    // Function definition
{
    LinkedListNode <int> *delNode = node -> next ;    // Initialize pointer with next value of node
    node -> data = delNode -> data;    // Assign data of next node to node
    node -> next = delNode -> next;    // Assign next of next node to node
    delete(delNode) ;    // Delete the next node
    return ;
}
===================================
