...
string fourSum(vector<int> &arr, int target, int n)    // function for four sum
{
    sort(arr.begin() , arr.end() ) ;    // sort the elements of given array.
    int low, high, reqSum, currSum;
    for(int i = 0 ; i < n ; i++)
    {
        for(int j = i + 1 ; j < n ; j++)
        {
            reqSum = target - arr[i] - arr[j] ;    // find reqSum
            low = j + 1 ; high = n - 1 ;
            while(low < high)
            {
                currSum = arr[low] + arr[high] ;    // find currSum
                if(currSum == reqSum)   return "Yes" ;    // check if currSum and reqSum are equal or not
