Node<int>* sortTwoLists(Node<int>* first, Node<int>* second)    // Defining function sortTwoLists()
{
    if(first == NULL) return second;    // Check if first list is empty
    if(second == NULL) return first;    // Check if second list is empty

    Node <int> *dummy = new Node <int> (-1) ;    // Initializing dummy node
    Node <int> *tail = dummy ;    // Initializing tail variable

    Node<int> *p = first ;  Node <int> *q = second ;    // Initializing p and q variables

    while(p != NULL && q != NULL)    // while loop
    {
        if(p -> data < q -> data)    // Check if nodes of p has data less than q
        {
            tail -> next = p ;  tail = p ;    // update the node of tail
            p = p -> next ;
        }
        else
        {
            tail -> next = q ;  tail = q ;    // update the node of tail
            q = q -> next ;
        }
    }

    tail -> next = (p == NULL) ? q : p ;    // update the node of tail

    return dummy -> next ;    // return dummy node of sorted list
}
