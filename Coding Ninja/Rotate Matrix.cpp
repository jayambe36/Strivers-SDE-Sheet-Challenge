...
{
    int val ;    // int variable to store return value
    int startRow = 0 ;    // define start row as 0
    int startCol = 0 ;    // define start col as 0
    int endRow = m - 1 ;  // define end row
    int endCol = n - 1 ;  // define end col

    while(startRow < endRow && startCol < endCol)    // While loop to traverse the matrix
    {
        val = mat[startRow][startCol] ;    // assign val as first value of matrix

        for(int index = startCol + 1 ; index <= endCol ; index++)
        {
            int temp = mat[startRow][index] ; 
            mat[startRow][index] = val ; 
            val = temp ;
        }
        ...
        {
            int temp = mat[index][endCol] ; 
            mat[index][endCol] = val ; 
            val = temp ; 
        ...
        {   
            int temp = mat[endRow][index] ;  
            mat[endRow][index] = val ; 
            val = temp ; 
        ...
        for(int index = endRow ; index >= startRow ; index--)
        {
            ...
            int temp = mat[index][startCol] ; 
            mat[index][startCol] = val ; 
            val = temp ; 

        }

        startCol++ ;  
        mat[startRow - 1][startCol - 1] = val ;    // assign val as last value of matrix
    }   
